<template>
    <div class="main">

        <ul class="graph-list">
            <li v-for="(item, index) of this.$store.state.graphs" :key="index" @click="OpenGraph(item)">
                {{item.title}}
            </li>
            <div class="add-new-graph" @click="AddNewGraph">
                <span></span>
            </div>
        </ul>

        <NewGraphModal :show="GraphModal.status" @closeuserinfomodal="CloseUserInfoModal"
        @saveorupdate="SaveOrUpdateUser" :items="GraphModal.data"></NewGraphModal>
    </div>
</template>

<script>
import NewGraphModal from '../components/category/newgraph'

export default {
    name: 'graphlist',
    data () {
        return {
            GraphModal: {
                status: false,
                data: null
            }
        }
    },
    components: {
        NewGraphModal
    },
    methods: {
        AddNewGraph: function() {
            this.GraphModal.status = true;
        },
        OpenGraph: function() {
            this.$router.push({ path: '/networkgraph' });
        },
        CloseUserInfoModal: function() {
            this.GraphModal.status = false;
        },
        SaveOrUpdateUser: function() {
            this.GraphModal.status = false;
            this.$router.push({ path: '/newgraph' });
        }
    }
}
</script>

<style>
.graph-list{
    list-style: none;
    text-align: right;
    padding: 20px;
    direction: rtl;
}

.graph-list li{
    height: 100px;
    width: 250px;
    float: right;
    margin-left: 20px;
    margin-bottom: 20px;
    border-radius: 6px;
    background: #ddd;
    padding: 20px;
    position: relative;
    cursor: pointer;
    transition: all .3 ease;
    color: #444;
    font-size: 14px;
    font-family: 'IRANSANS BOLD';
    line-height: 65px;
    text-align: center;
}

.graph-list li:hover{
    background: #f3b233;
    transition: all .3 ease;
}

.add-new-graph{
    text-align: center;
    float: right;
    height: 100px;
    width: 150px;
    transition: all .3s ease;
    cursor: pointer;
    position: relative;
    background: #ddd;
    border-radius: 6px;
    margin-left: 20px;
    margin-bottom: 20px;
}

.add-new-graph span{
    position: absolute;
    top: 50%;
    left: 50%;
    height: 40px;
    width: 40px;
    background-repeat: no-repeat;
    background-size: 40px;
    transform: translate(-50%,-50%);
    background-image: url(data:image/svg+xml;utf8;base64,);
}

.add-new-graph:hover{
    color: #ccc;
    transition: all .3s ease;
}
</style>

