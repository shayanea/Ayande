<template>
    <div v-if="this.$store.state.showcreateview">
        ...
        <!-- Type Popup -->
        <ViewType :show="showtype" :model="list" @close="Close" @showfilter="ShowConfig"></ViewType>
        ...
        <!-- Config Popup -->
        <Config :show="showconfig" :active="items.model" :model="list" @hideconfig="HideConfig" @showsubmit="SubmitView"></Config>
...
export default {
    ...
    methods: {
        ...
        SubmitView: function(object) {
            ...
            this.$store.commit('ShowCreateView');
            this.$store.commit('ShowBgWrapper');
            this.$router.push({ path: '/defaultview' });
        }
    }
}
</script>
